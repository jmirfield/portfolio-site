class Cell {
    #isAlive;
    constructor() {
        this.#isAlive = Math.random() > .95;
    }

    get isAlive() {
        return this.#isAlive;
    }

    kill() {
        this.#isAlive = false;
    }

    resurrect() {
        this.#isAlive = true;
    }
}


class GameOfLife {
    #canvas;
    #ctx;
    #rows;
    #cols;
    #width;
    #height;
    #matrix;
    #debug;
    constructor(canvas, n, debug = false) {
        this.#canvas = canvas;
        this.#ctx = canvas.getContext('2d');
        this.#rows = n;
        this.#cols = n;
        this.#width = canvas.width / n;
        this.#height = canvas.height / n;
        this.#matrix = Array(n).fill().map(row => Array(n));
        this.#debug = debug;
        this.startGame();
    }

    startGame() {
        for (let i = 0; i < this.#rows; i++) {
            for (let t = 0; t < this.#cols; t++) {
                this.#matrix[i][t] = new Cell();
            }
        }
    }

    get matrix() {
        return this.#matrix
    }

    #drawGrid() {
        const c = this.#ctx;
        const canvas = this.#canvas;
        for (let i = 0; i < this.#rows; i++) {
            c.beginPath();
            c.moveTo(this.#width * i, 0);
            c.lineTo(this.#width * i, canvas.height);
            c.stroke();

            c.beginPath();
            c.moveTo(0, this.#height * i);
            c.lineTo(canvas.width, this.#height * i)
            c.stroke();
        }
    }

    #fillOne(x, y) {
        const c = this.#ctx;
        const canvas = this.#canvas;
        c.beginPath();
        c.fillRect(x * this.#width, y * this.#height, this.#width, this.#height);
        c.stroke();
    }

    #matrixCheck() {
        const initial = this.#matrix.map((arr) => {
            return arr.slice();
        });


        for (let i = 0; i < this.#rows; i++) {
            for (let t = 0; t < this.#cols; t++) {
                const count = this.#checkNeighbors(i, t, initial);
                if (count < 2 || count > 3) this.#matrix[i][t].kill()
                if (count === 3) this.#matrix[i][t].resurrect()
            }
        }
    }

    #cellHelper(x, y) {
        return this.#matrix[x][y].isAlive ? 1 : 0;
    }

    #checkNeighbors(x, y, matrix) {
        let count = 0;
        try {
            count += this.#cellHelper(x-1, y-1)
            count += this.#cellHelper(x, y-1)
            count += this.#cellHelper(x+1, y-1)
            count += this.#cellHelper(x-1, y)
            count += this.#cellHelper(x+1, y)
            count += this.#cellHelper(x-1, y+1)
            count += this.#cellHelper(x, y+1)
            count += this.#cellHelper(x+1, y+1)
        } catch (e) {
            //Catching indices outside of matrix, will make cleaner solution in due time
        }

        return count;
    }



    #drawCells() {
        for (let i = 0; i < this.#rows; i++) {
            for (let t = 0; t < this.#cols; t++) {
                if (this.#matrix[i][t].isAlive) {
                    this.#fillOne(i, t)
                }
            }
        }
    }

    draw() {
        if (this.#debug) {
            this.#drawGrid();
        }
        this.#matrixCheck();
        this.#drawCells();
    }

}

export default GameOfLife;